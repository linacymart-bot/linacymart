...
async function searchImage(query) {
  return new Promise((resolve) => {
    gis(query + ' bf suma', (error, results) => {
      if (error || !results || results.length === 0) {
        resolve(null);
      } else {
        // Return first valid https image link
        const img = results.find(r => r.url.startsWith('https://') && (r.url.includes('.jpg') || r.url.includes('.png') || r.url.includes('.jpeg') || r.url.includes('.webp')));
        resolve(img ? img.url : results[0].url);
      }
    });
  });
}
...
async function run() {
  ...
  for (const prod of products) {
    ...
    const url = await searchImage(prod.name);
    ...
    if (success) {
      ...
      if (product) {
        ...
        // First try updating placeholder.svg
